    ...
    reader.onload = async (event) => {
      try {
        ...
        if (!json.posts || !Array.isArray(json.posts)) {
          alert('تنسيق الملف غير صحيح. يجب أن يحتوي على مصفوفة "posts".');
          return;
        }

        const postsToImport: Post[] = json.posts.map((p: any, index: number) => ({
          id: '', // Will be generated by DB
          title: p.title || `منشور مستورد ${index + 1}`,
          content: p.content || '',
        ...
        if (!window.confirm(`سيتم استيراد ${postsToImport.length} منشور. هل تريد المتابعة؟`)) {
          return;
        }
